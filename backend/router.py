...
#create router that takes all items
#always 2 atributes, PATH and RESPONSE
@router.get("/prooducts/", response_model=List[ProductResponse])
def read_all_products(db: Session = Depends(get_db)):
    return get_products(db=db)

#create router that takes 1 item
@router.get("/prooducts/{product_id}", response_model=ProductResponse)
